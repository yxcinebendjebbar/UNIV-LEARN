...
async function deleteCourseFiles(course) {
  try {
    ...
    const uploadsPath = path.join("uploads", course.userId.toString(),course.courseName).replace(/\\/g, "/");
...
router.put("/users/profile-picture/:userId", isAdmin ,upload1.single("profilePicture"), async (req, res) => {
  try {
...
router.post(
    ...
    async (req, res) => {
      try {
        const { userId, name, description, specialty, faculty, department, level } =
